...
static u8* cumulative = NULL;
static void symfusion_results(afl_forkserver_t *fsrv, u32* count, u32* new_cov) {

  if (cumulative == NULL)
    cumulative = malloc(map_size);

  u32 i;
  for (i = 0; i < map_size; i++) {

    if ((fsrv->trace_bits[i] | cumulative[i]) != cumulative[i] ) {
      
      *count += 1;
      if (cumulative[i] == 0)
        *new_cov += 1;

      cumulative[i] |= fsrv->trace_bits[i];
    }
...
#define SYMFUSION 1
#if SYMFUSION
  int fd_pipe_read = -1, fd_pipe_write = -1, r, data;
  char* pipe_name_read = getenv("SYMFUSION_TRACER_PIPE_READ");
  char* pipe_name_write = getenv("SYMFUSION_TRACER_PIPE_WRITE");
  if (pipe_name_read && pipe_name_write) {
    ACTF("Opening pipes with SymFusion");
    fd_pipe_read = open(pipe_name_read, O_RDONLY);
    fd_pipe_write = open(pipe_name_write, O_WRONLY);
  }
loop:
  if (fd_pipe_read >= 0 && fd_pipe_write >= 0) {
    ACTF("Writing to pipe with SymFusion");
    data = 0xCAFECAFE;
    r = write(fd_pipe_write, &data, sizeof(data));
    ACTF("Reading from pipe with SymFusion");
    r = read(fd_pipe_read, &data, sizeof(data));
    ACTF("pipe read: res=%d data=%x", r, data);
  }
#endif
  ...
  for (i = 0; i < (u32)nl_cnt; ++i) {
    ...
    if (read_file(fn2)) {
      ...
      if (fd_pipe_read >= 0 && fd_pipe_write >= 0) {
        u32 counters[2] = { 0 };
        symfusion_results(fsrv, &counters[0], &counters[1]);
        int fd = open(outfile, O_WRONLY | O_CREAT | O_EXCL, DEFAULT_PERMISSION);
        ck_write(fd, counters, sizeof(counters), outfile);
        close(fd);
      } else {
        if (collect_coverage)
...
#if SYMFUSION
  if (fd_pipe_read >= 0 && fd_pipe_write >= 0) goto loop;
#endif
